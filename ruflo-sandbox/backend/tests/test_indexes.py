...
def test_index_usage(db_session):
    ...
    if dialect == "sqlite":
        ...
        result = db_session.execute(
            text("EXPLAIN QUERY PLAN SELECT * FROM patients WHERE tenant_id = 1")
        ).fetchall()
        plan = str(result)
        ...
        # Postgres/Other
        result = db_session.execute(
            text("EXPLAIN SELECT * FROM patients WHERE tenant_id = 1")
        ).fetchall()
        plan = str(result)
        found_index = False
        for row in result:
    ...
    assert found_index, (
        f"Query did NOT use the expected index! (Dialect: {dialect}) Plan: {plan}"
    )
