...
TEST_CASES = [
    ...
    (
        "create_patient",
        "سجلي مريض جديد اسمه محمود حسن سنه 30 سنة وتليفه 0100000000",
        ["response"],
    ),
    ...
    (
        ...
        "احجزي ميعاد لمنى زكي الثلاثاء الجاي الساعة 5",
        ["create_appointment"],
    ),
    (
        "create_appointment",
        "حجز لمريض اسمه علي محمود يوم 2025-10-01 الساعة 10 صباحا",
        [],
    ),
    ...
    (
        ...
        "المريض بيشتكي من ألم في الجهة اليمنى، تم عمل أشعة وتبين وجود خراج",
        [],
    ),
...
async def run_tests():
    ...
    for expected_tool, query, allowed_alternatives in TEST_CASES:
        ...
        try:
            ...
            is_success = (actual_tool == expected_tool) or (
                actual_tool in allowed_alternatives
            )

            # Allow 'response' if it's asking for confirmation but correctly identified parameters
            if (
                not is_success
                and expected_tool == "create_patient"
                and actual_tool == "response"
            ):
                if result.get("_intent") in [
                    "patient_registration",
                    "patient_registration_confirm",
                ]:
                    is_success = True
