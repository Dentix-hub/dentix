...
from backend.schemas.inventory import BatchBase
...
material_id = 8
warehouse_id = (
    1  # Force adding to MAIN warehouse to ensure availability if fallback logic fails
)
...
batch = (
    db.query(Batch)
    .filter(Batch.material_id == material_id, Batch.batch_number == "dfsdhd")
    .first()
)
...
if not batch:
    ...
    batch_data = BatchBase(
        batch_number="FIX001", expiry_date=date(2027, 1, 1), cost_per_unit=100.0
    )
else:
