...
def seed_data(db: Session):
    ...
    if not super_admin:
        ...
        system_tenant = (
            db.query(models.Tenant).filter(models.Tenant.name == "System Admin").first()
        )
        ...
        if not existing_admin:
            ...
            if not admin_pass_env:
                if env == "production":
                    print(
                        "CRITICAL: ADMIN_PASSWORD not set in production. Skipping admin creation."
                    )
                    return
                else:
                    print(
                        "WARNING: ADMIN_PASSWORD not set. Using default dev password."
                    )
