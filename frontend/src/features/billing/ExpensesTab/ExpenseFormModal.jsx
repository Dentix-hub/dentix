...
export default function ExpenseFormModal({ isOpen, onClose, newExpense, setNewExpense, handleCreateExpense }) {
    ...
    return (
        ...
        >
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full p-3 bg-surface border border-border rounded-xl outline-none focus:border-primary transition-colors text-text-primary text-sm font-medium"
                        >
                ...
                <div>
                    <label className="block text-sm font-bold text-text-primary mb-1">{t('billing.expenses.notes', 'Notes')}</label>
                    <textarea
                        value={newExpense.notes || ''}
                        onChange={e => setNewExpense({ ...newExpense, notes: e.target.value })}
                        placeholder={t('billing.expenses.notes_placeholder', 'Optional expense notes...')}
                        className="w-full p-3 bg-surface border border-border rounded-xl outline-none focus:border-primary transition-colors text-text-primary text-sm font-medium resize-none h-20"
                    />
                </div>
