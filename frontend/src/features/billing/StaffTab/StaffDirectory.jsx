...
import { Card, EmptyState, Button, SkeletonBox } from '@/shared/ui';
import { useTranslation } from 'react-i18next';

export default function StaffDirectory({ staff, staffLoading, roleLabels, openStaffProfile }) {
                        ...
                        <tbody className="divide-y divide-border">
                            {staff.map(s => (
                                <tr key={s.id} className="hover:bg-surface-hover transition-all">
                                    ...
                                    <td className="p-4">
                                        <span className="px-3 py-1 bg-surface-hover rounded-lg text-xs font-bold text-text-secondary">
