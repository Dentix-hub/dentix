...
if (-not (Test-Path $envFile)) {
    if (Test-Path $envExample) {
        Write-Host "  [WARN] No .env found. Copying from .env.dev.example..." -ForegroundColor Yellow
        Copy-Item $envExample $envFile
        Write-Host "  [INFO] Please edit .env with your actual values before continuing." -ForegroundColor Yellow
        Write-Host "  [INFO] At minimum, set DATABASE_URL and ENCRYPTION_KEY." -ForegroundColor Yellow
        exit 1
    } else {
        Write-Host "  [ERROR] No .env or .env.dev.example found!" -ForegroundColor Red
        exit 1
    }
} else {
...
if ($dbUrlLine -like "*$prodDbRef*") {
    Write-Host ""
    Write-Host "  [ERROR] DATABASE ISOLATION VIOLATION!" -ForegroundColor Red
    ...
    Write-Host "  [OK] Security Check Passed: No production database URL detected." -ForegroundColor Green
}

# 4. Start services
Write-Host ""
Write-Host "[4/4] Starting Dentix services..." -ForegroundColor Yellow
Write-Host ""
