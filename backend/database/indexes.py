        ...
        for table, indexes in INDEX_DEFINITIONS.items():
            for index_name, columns in indexes:
                try:
                    ...
                    """)
                    result = conn.execute(check_sql, {"table": table, "index": index_name}).fetchone()
                    ...
                    create_sql = text(f"CREATE INDEX IF NOT EXISTS {index_name} ON {table} ({col_str})")
    ...
    try:
        with engine.connect() as conn:
            ...
            for q in queries:
                ...
                print(f"Calls: {q.calls}, Avg: {q.mean_time_ms}ms, Total: {q.total_time_ms}ms")
