...
for row in cursor.fetchall():
    print(
        f"  ID={row['id']}, Name={row['name']}, Type={row['type']}, Tenant={row['tenant_id']}"
    )
...
if rows:
    for row in rows:
        print(
            f"  Session ID={row['session_id']}, Status={row['status']}, Material={row['material_name']} (ID={row['material_id']}), Type={row['material_type']}, StockQty={row['stock_qty']}"
        )
else:
...
cursor.execute(
    "SELECT id, name, type FROM materials WHERE name LIKE '%كومبوزيت%' OR name LIKE '%Composite%' OR name LIKE '%composite%'"
)
...
if composite_rows:
    for row in composite_rows:
        ...
        # Check batches
        cursor.execute(
            "SELECT id, batch_number FROM batches WHERE material_id = ?", (row["id"],)
        )
        ...
        for b in batches:
            ...
            # Check stock items
            cursor.execute(
                "SELECT id, quantity FROM stock_items WHERE batch_id = ?", (b["id"],)
            )
            ...
            for si in items:
                ...
                # Check sessions
                cursor.execute(
                    "SELECT id, status FROM material_sessions WHERE stock_item_id = ?",
                    (si["id"],),
                )
