...
def test_calc():
    ...
    for doc in analytics:
        print(
            f"Dr. {doc['doctor_name']} | Revenue: {doc['revenue']} | Collected: {doc['collected']} | Lab: {doc['lab_cost']} | %: {doc['commission_percent']}"
        )
    ...
    for d in dues:
        print(
            f"Dr. {d['name']} | Base (Coll-Lab): {d['commission_base']} | Calc Due: {d['total_due']}"
        )

        # Verify Formula
        expected = (d["collected"] - d["lab_cost"]) * (
