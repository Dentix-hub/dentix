...
def test_double_encryption():
    ...
    try:
        decrypted_view = manager.decrypt(
            encrypted_with_lost_key, allow_plaintext_fallback=True
        )
    ...
    if re_decrypted == encrypted_with_lost_key:
        ...
        print(
            "   This confirms the cycle: Lost Key -> Read Ciphertext -> Save -> Double Encrypt."
        )
    else:
