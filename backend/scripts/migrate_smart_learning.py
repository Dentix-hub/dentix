...
from backend.models import inventory # Import to register models
...
def migrate():
    ...
    with engine.connect() as conn:
        if 'closed_at' not in columns:
            ...
            conn.execute(text("ALTER TABLE material_sessions ADD COLUMN closed_at DATETIME"))
        
        if 'total_amount_consumed' not in columns:
            print("Adding 'total_amount_consumed' to material_sessions...")
            conn.execute(text("ALTER TABLE material_sessions ADD COLUMN total_amount_consumed FLOAT"))
