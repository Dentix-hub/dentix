...
try:
    ...
    tenant = db.query(models.Tenant).filter(models.Tenant.id == admin_user.tenant_id).first()
    if tenant and tenant.subscription_plan:
        max_users = tenant.subscription_plan.max_users
        print(f"Max Users: {max_users}")
        if max_users is not None:
            current_count = db.query(models.User).filter(models.User.tenant_id == admin_user.tenant_id).count()
