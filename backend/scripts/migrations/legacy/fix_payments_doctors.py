...
def fix_payments_doctors():
    ...
    try:
        ...
        if not has_doctor_id:
            ...
            try:
                with engine.connect() as conn:
                    conn.execute(
                        text(
                            "ALTER TABLE payments ADD COLUMN doctor_id INTEGER REFERENCES users(id)"
                        )
                    )
                    conn.commit()
        ...
        payments_without_doctor = (
            ...
            .filter(
                or_(models.Payment.doctor_id.is_(None), models.Payment.doctor_id == 0)
            )
            .all()
        )
        ...
        for payment in payments_without_doctor:
            ...
            if not tenant_id:
                patient = (
                    ...
                    .filter(models.Patient.id == payment.patient_id)
                    .first()
                )
                if patient:
                    ...
                    if patient_tenant_id:
                        ...
                        print(
                            f"[WARNING] Payment {payment.id} has no tenant_id and patient {patient.id} has no tenant_id. Skipping."
                        )
                        continue
                else:
                    print(
                        f"[WARNING] Payment {payment.id} references non-existent patient {payment.patient_id}. Skipping."
                    )
                    continue
            ...
            if recent_treatment and recent_treatment.doctor_id:
                ...
                if tenant_id:
                    ...
                    if doctor:
                        ...
                        fixed_count += 1
                        print(
                            f"  Assigned payment {payment.id} to doctor {doctor.username} (from tenant default)"
                        )
                    else:
                        print(
                            f"[WARNING] No doctor found for payment {payment.id}. Skipping."
                        )
