...
def fix_treatments_doctors():
    ...
    try:
        ...
        if not has_tenant_id:
            ...
            try:
                with engine.connect() as conn:
                    conn.execute(text("ALTER TABLE treatments ADD COLUMN tenant_id INTEGER"))
                    conn.commit()
        ...
        treatments_without_doctor = db.query(models.Treatment).filter(
            or_(
        ...
        for treatment in treatments_without_doctor:
            ...
            if not tenant_id:
                patient = db.query(models.Patient).filter(
                    models.Patient.id == treatment.patient_id
                ).first()
                if patient:
                    ...
                    if patient_tenant_id:
                        ...
                        print(f"[WARNING] Treatment {treatment.id} has no tenant_id and patient {patient.id} has no tenant_id. Skipping.")
                        continue
                else:
                    print(f"[WARNING] Treatment {treatment.id} references non-existent patient {treatment.patient_id}. Skipping.")
                    continue
            ...
            if tenant_id not in tenant_doctor_map:
                ...
                if doctor:
                    ...
                    print(f"  Assigning treatments for tenant {tenant_id} to doctor: {doctor.username} (ID: {doctor.id})")
                else:
                    # If no doctor found, skip
                    print(f"  [WARNING] No doctor found for tenant {tenant_id}. Skipping treatments for this tenant.")
        ...
        # 4. Show summary
        remaining = db.query(models.Treatment).filter(
            or_(
                models.Treatment.doctor_id.is_(None),
                models.Treatment.doctor_id == 0
            )
