...
PERSIST_DIRECTORY = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "rag_storage"
)
...
class KnowledgeStoreInterface:
    ...
    def add(
        self, text: str, tenant_id: int, source: str = "manual", metadata: Dict = None
    ) -> str:
        pass

    def search(
        self, query: str, tenant_id: int, n_results: int = 2
    ) -> List[Dict[str, Any]]:
        pass
...
class RealKnowledgeStore(KnowledgeStoreInterface):
    ...
    def __init__(self):
        ...
        self.collection = self.client.get_or_create_collection(
            name="clinic_knowledge", embedding_function=self.embedding_fn
        )
    ...
    def search(
        ...
        results = self.collection.query(
            query_texts=[query], n_results=n_results, where={"tenant_id": tenant_id}
        )
        formatted_results = []
