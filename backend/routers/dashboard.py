...
async def get_dashboard_stats(
    ...
    return await _get_cached_dashboard_stats(current_user.tenant_id, doctor_id, db)


@cached(key_prefix="dashboard_stats", expire=120)
async def _get_cached_dashboard_stats(tenant_id: int, doctor_id: int, db: AsyncSession):
    """Internal helper to ensure tenant_id and doctor_id are part of the cache key."""
    data = await crud.get_dashboard_stats(db, tenant_id, doctor_id=doctor_id)
...
async def get_finance_stats(
    ...
    doctor_id = current_user.id if current_user.role == "doctor" else None
    return await _get_cached_finance_stats(current_user.tenant_id, doctor_id, db)


@cached(key_prefix="finance_stats", expire=60)
async def _get_cached_finance_stats(tenant_id: int, doctor_id: int, db: AsyncSession):
    """Internal helper to ensure tenant_id and doctor_id are part of the cache key."""
    data = await crud.get_financial_stats(db, tenant_id, doctor_id=doctor_id)
    return success_response(data=data)
