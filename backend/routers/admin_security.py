...
def block_ip(
    ...
    return SecurityService.block_ip(db, ip_data.ip_address, ip_data.reason, current_user.username)
...
async def trigger_test_job(
    ...
    # Start job record
    job = JobService.start_job(db, "manual_test_job", triggered_by=current_user.username)
