...
def debug_procedures(db: Session = Depends(get_db)):
    ...
    try:
        ...
        global_procs = db.execute(text("SELECT COUNT(*) FROM procedures WHERE tenant_id IS NULL")).scalar()
        ...
        price_list_item_count = db.execute(text("SELECT COUNT(*) FROM price_list_items")).scalar()
