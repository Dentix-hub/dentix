...
def get_feedback_messages(
    ...
    messages = db.query(models.SupportMessage).order_by(models.SupportMessage.created_at.desc()).all()
    
    # Attach names for display
    for msg in messages:
        user = db.query(models.User).filter(models.User.id == msg.user_id).first()
        tenant = db.query(models.Tenant).filter(models.Tenant.id == msg.tenant_id).first()
