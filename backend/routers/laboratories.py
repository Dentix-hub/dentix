...
def _get_cached_laboratories(db: Session, tenant_id: int):
    return db.query(models.Laboratory).filter(
        models.Laboratory.tenant_id == tenant_id
    ).order_by(models.Laboratory.name).all()
...
def get_laboratory(
    ...
    lab = db.query(models.Laboratory).filter(
...
def update_lab_order(
    ...
    linked_treatment = db.query(models.Treatment).filter(
...
def get_patient_lab_orders(
    ...
    orders = db.query(models.LabOrder).filter(
        models.LabOrder.patient_id == patient_id
    ).order_by(models.LabOrder.order_date.desc()).all()
...
def get_lab_orders_stats(
    ...
    in_progress_orders = base_query.filter(models.LabOrder.status == "in_progress").count()
    completed_orders = base_query.filter(models.LabOrder.status == "completed").count()
    
    total_cost = db.query(func.sum(models.LabOrder.cost)).filter(
        models.LabOrder.tenant_id == current_user.tenant_id
    ).scalar() or 0
    
    total_revenue = db.query(func.sum(models.LabOrder.price_to_patient)).filter(
        models.LabOrder.tenant_id == current_user.tenant_id
    ).scalar() or 0
    
    total_labs = db.query(models.Laboratory).filter(
        models.Laboratory.tenant_id == current_user.tenant_id,
...
def get_lab_stats(
    ...
    # 2. Payments Stats
    total_paid = db.query(func.sum(models.LabPayment.amount)).filter(
...
def get_lab_payments(
    ...
    payments = db.query(models.LabPayment).filter(
        models.LabPayment.laboratory_id == lab_id,
        models.LabPayment.tenant_id == current_user.tenant_id
    ).order_by(models.LabPayment.date.desc()).all()
    return payments
