...
def create_warehouse(
    ...
    return inventory_service.create_warehouse(warehouse, current_user.tenant_id or 1, db)
...
def delete_warehouse(
    ...
    try:
        inventory_service.delete_warehouse(warehouse_id, current_user.tenant_id or 1, db)
...
def update_material(
    ...
    try:
        return inventory_service.update_material(material_id, data, current_user.tenant_id or 1, db)
    except ValueError as e:
...
def get_stock_summary(
    ...
    return inventory_service.get_material_stock_summary(current_user.tenant_id or 1, warehouse_id, db)
...
def get_expiry_alerts(
    ...
    """
    return inventory_service.get_expiry_alerts(current_user.tenant_id or 1, days=days, db=db)
...
def open_session(
    ...
    try:
        session = inventory_service.open_session(data.stock_item_id, current_user.id, db)
        return session
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def close_material_session(
    ...
    try:
        ...
        session = db.query(inv_models.MaterialSession).options(
        ...
        if session.status == "CLOSED":
            return {"success": True, "message": "Session already closed", "already_closed": True}
            
        # Get material type
        material = session.stock_item.batch.material if session.stock_item and session.stock_item.batch else None
        ...
        if material_type == "DIVISIBLE":
            ...
            learning_service.close_session(session_id, float(total_consumed), current_user.id)
...
@router.get("/sessions/active", response_model=List[schemas.inventory.MaterialSessionRead])
def get_active_sessions(
    ...
    sessions = db.query(inv_models.MaterialSession).options(
        joinedload(inv_models.MaterialSession.stock_item)
        .joinedload(inv_models.StockItem.batch)
        .joinedload(inv_models.Batch.material)
    ).join(
...
def delete_procedure_weight(
    ...
    weight = db.query(inv_models.ProcedureMaterialWeight).filter(inv_models.ProcedureMaterialWeight.id == weight_id).first()
    if not weight:
...
def set_procedure_weight(
    ...
    proc = db.query(clinical_models.Procedure).filter(
        clinical_models.Procedure.name == data.procedure_name,
        (clinical_models.Procedure.tenant_id == tenant_id) | (clinical_models.Procedure.tenant_id == None)
    ).first()
    ...
    weight_obj = db.query(inv_models.ProcedureMaterialWeight).filter(
...
def get_procedure_weights(
    ...
    if material_id:
        query = query.filter(inv_models.ProcedureMaterialWeight.material_id == material_id)
        
    if procedure_id:
        query = query.filter(inv_models.ProcedureMaterialWeight.procedure_id == procedure_id)
        
    return query.all()

@router.get("/materials/{material_id}/stock", response_model=List[schemas.inventory.StockItemRead])
def get_material_stock(
    ...
    items = db.query(inv_models.StockItem).join(
        inv_models.Batch
    ).filter(
        inv_models.Batch.material_id == material_id,
        inv_models.StockItem.tenant_id == tenant_id,
        (inv_models.StockItem.quantity > 0) | (inv_models.StockItem.id.in_(subquery))
    ).all()
    return items
