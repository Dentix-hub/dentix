    ...
    try:
        ...
        if "failed_login_attempts" not in columns:
            schema_changes.append(
                {"table": "users", "def": "failed_login_attempts INTEGER DEFAULT 0"}
            )
        if "account_locked_until" not in columns:
            schema_changes.append(
                {"table": "users", "def": "account_locked_until TIMESTAMP"}
            )
        if "last_failed_login" not in columns:
            schema_changes.append(
                {"table": "users", "def": "last_failed_login TIMESTAMP"}
            )
        if "is_2fa_enabled" not in columns:
            schema_changes.append(
                {"table": "users", "def": "is_2fa_enabled BOOLEAN DEFAULT FALSE"}
            )
        if "otp_secret" not in columns:
            schema_changes.append({"table": "users", "def": "otp_secret VARCHAR"})
        ...
        if "logo" not in t_columns:
            schema_changes.append({"table": "tenants", "def": "logo VARCHAR"})
        if "grace_period_until" not in t_columns:
            schema_changes.append(
                {"table": "tenants", "def": "grace_period_until TIMESTAMP"}
            )
        if "auto_suspend_at" not in t_columns:
            schema_changes.append(
                {"table": "tenants", "def": "auto_suspend_at TIMESTAMP"}
            )
        if "payment_failed_count" not in t_columns:
            schema_changes.append(
                {"table": "tenants", "def": "payment_failed_count INTEGER DEFAULT 0"}
            )
        if "manual_override_reason" not in t_columns:
            schema_changes.append(
                {"table": "tenants", "def": "manual_override_reason VARCHAR"}
            )
        if "last_login" not in t_columns:
            schema_changes.append({"table": "tenants", "def": "last_login TIMESTAMP"})
        if "is_deleted" not in t_columns:
            schema_changes.append(
                {"table": "tenants", "def": "is_deleted BOOLEAN DEFAULT FALSE"}
            )
        if "deleted_at" not in t_columns:
            schema_changes.append({"table": "tenants", "def": "deleted_at TIMESTAMP"})
        ...
        if "is_deleted" not in nr_columns:
            schema_changes.append(
                {
                    "table": "notification_reads",
                    "def": "is_deleted BOOLEAN DEFAULT FALSE",
                }
            )

        # 1.8 Inspect Subscription Plans
        sp_columns = [c["name"] for c in inspector.get_columns("subscription_plans")]
        if "is_ai_enabled" not in sp_columns:
            schema_changes.append(
                {
                    "table": "subscription_plans",
                    "def": "is_ai_enabled BOOLEAN DEFAULT FALSE",
                }
            )
        if "ai_daily_limit" not in sp_columns:
            schema_changes.append(
                {
                    "table": "subscription_plans",
                    "def": "ai_daily_limit INTEGER DEFAULT 0",
                }
            )
        if "ai_features" not in sp_columns:
            schema_changes.append(
                {"table": "subscription_plans", "def": "ai_features TEXT"}
            )
        ...
        if "subscription_payments" in inspector.get_table_names():
            ...
            if "paid_by" not in pay_columns:
                schema_changes.append(
                    {"table": "subscription_payments", "def": "paid_by VARCHAR"}
                )
        else:
            log(
                "Table 'subscription_payments' does not exist yet (should be created by auto-migration)."
            )
