...
def create_treatment(
    ...
    if treatment.consumedMaterials:
        try:
            ...
            for item in treatment.consumedMaterials:
                ...
                if not is_valid:
                    errors.append(f"{mat_name} (Need: {item.quantity}, Available: {available})")
            
            if errors:
                raise HTTPException(status_code=400, detail="فشل حفظ العلاج بسبب نقص المخزون: " + " | ".join(errors))
    ...
    procedure_obj = db.query(models.Procedure).filter(
