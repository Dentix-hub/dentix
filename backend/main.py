...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Automatic Unique Constraints Relaxation (For Online Deployment)
        from backend import migrate_unique_constraints
        print("[STARTUP] Applying Unique Constraints Relaxation...")
        # migrate_unique_constraints.run_migration()

        # Phase 0 Emergency: Soft Delete Migration
        from backend import migrate_soft_delete_v2
        print("[STARTUP] Applying Soft Delete Migration...")
        # migrate_soft_delete_v2.migrate_soft_delete()

        # Phase 0.1 Emergency: Fix NULL values
        from backend import fix_soft_delete_nulls
        print("[STARTUP] Applying Data Integrity Fix...")
        # fix_soft_delete_nulls.fix_null_values()

        # Phase 0.2: Automatic Subscription Schema Fix (Production Patch)
        try:
             from sqlalchemy import text
             db_patch = database.SessionLocal()
             try:
                 print("[STARTUP] Checking Subscription Plans Schema...")
                 # Robust check for SQLite/Postgres transparency
                 try:
                     db_patch.execute(text("ALTER TABLE subscription_plans ADD COLUMN is_default BOOLEAN DEFAULT FALSE;"))
                     print("[STARTUP] SUCCESS: Added 'is_default' column to subscription_plans")
                     db_patch.commit()
                 except Exception as patch_err:
                     if "duplicate" in str(patch_err).lower() or "exists" in str(patch_err).lower():
                         print("[STARTUP] Schema check passed (Column likely exists).")
                     else:
                         print(f"[STARTUP] Schema patch note: {patch_err}")
             finally:
                 db_patch.close()
        except Exception as e:
             print(f"[WARNING] Subscription schema patch failed: {e}")

    except Exception as e:
        print(f"[WARNING] Schema migration failed: {e}")

    # Phase 7.1: Missing Columns Patch (Staging Fix)
    try:
        from sqlalchemy import text
        db_patch_cols = database.SessionLocal()
        try:
             print("[STARTUP] Checking for missing columns in 'patients'...")
             # Check for email
             try:
                 db_patch_cols.execute(text("ALTER TABLE patients ADD COLUMN email VARCHAR;"))
                 print("✅ [STARTUP] Added missing column 'email' to 'patients'")
                 db_patch_cols.commit()
             except Exception as col_err:
                 if "duplicate" in str(col_err).lower() or "exists" in str(col_err).lower():
                     pass # Column exists
                 else:
                     print(f"⚠️ [STARTUP] Failed to add 'email': {col_err}")
             
             # Check for address
             try:
                 db_patch_cols.execute(text("ALTER TABLE patients ADD COLUMN address VARCHAR;"))
                 print("✅ [STARTUP] Added missing column 'address' to 'patients'")
                 db_patch_cols.commit()
             except Exception as col_err:
                 if "duplicate" in str(col_err).lower() or "exists" in str(col_err).lower():
                     pass 
                 else:
                     print(f"⚠️ [STARTUP] Failed to add 'address': {col_err}")

        finally:
             db_patch_cols.close()
    except Exception as e:
        print(f"[WARNING] Column patch failed: {e}")
...
# 2. Rate Limiting
# 2. Rate Limiting
# limiter is imported from backend.core.limiter
...
from backend.routers import (
    auth, patients, appointments, users, treatments,
    payments, expenses, laboratories, settings,
    procedures, dashboard, notifications,
    ai, ai_admin, ai_assist, support, system_admin, admin,
    admin_security, admin_features, admin_subscriptions, repair, accounting,
    password_reset, medications, prescriptions, analytics_ai_v2,
    health, admin_doctors,  # Health checks + Multi-Doctor Management
    price_lists, insurance, attachments, system_logs,  # Price Lists, Insurance, Attachments, System Logs
...
app.include_router(system_logs.router, prefix=f"{API_V1_STR}/system/logs", tags=["system-logs"])
...
app.include_router(admin_doctors.router, prefix=API_V1_STR)  # Multi-Doctor visibility management
...
async def catch_all(full_path: str):
    ...
    if full_path.startswith("api") or full_path.startswith("static") or full_path.startswith("uploads") or full_path.startswith("auth"):
...
async def startup_event():
    ...
    # ... rest of the startup logic is in lifespan, this is just for the log
    pass
...
def debug_db_info():
    ...
    try:
        ...
        # Check specific columns in users
        user_cols = [c['name'] for c in inspector.get_columns('users')]
        has_security_cols = 'failed_login_attempts' in user_cols
        ...
        return {
            ...
            "tables": tables,
            "user_columns_check": {
                "failed_login_attempts": 'failed_login_attempts' in user_cols,
                "account_locked_until": 'account_locked_until' in user_cols
            },
            "models_loaded_check": {
                "BlockedIP": hasattr(models, 'BlockedIP'),
                "LoginHistory": hasattr(models, 'LoginHistory')
            }
        }
    except Exception as e:
        return {"error": str(e)}
