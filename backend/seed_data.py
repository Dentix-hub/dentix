...
FIRST_NAMES = ["أحمد", "محمد", "محمود", "علي", "سارة", "منى", "نور", "ليلى", "خالد", "يوسف", "كريم", "هنا", "مريم", "عمر"]
LAST_NAMES = ["علي", "إبراهيم", "حسن", "السيد", "محمد", "عبدالله", "عثمان", "كامل", "زكي", "راضي"]
PROCEDURES = [
    ("كشف", 100),
    ("حشو عادي", 300),
    ("حشو عصب", 800),
...
def seed_data():
    ...
    try:
        ...
        for i, clinic_name in enumerate(CLINICS):
            ...
            roles = [("manager", "admin"), ("dr_ahmed", "doctor"), ("dr_sara", "doctor"), ("nurse", "assistant"), ("recep", "receptionist")]
            doctors = []
            ...
            for lab_name in LABS:
                lab = db.query(models.Laboratory).filter(models.Laboratory.name == lab_name, models.Laboratory.tenant_id == tenant.id).first()
                if not lab:
            ...
            for patient in patients:
                if random.random() > 0.7: # 30% of patients have lab work
                    ...
                    lab_order = models.LabOrder(
                        ...
                        work_type=random.choice(["تركيبة بورسلين", "طقم كامل", "زيركون"]),
                        cost=random.randint(500, 2000), # Lab cost
                        price_to_patient=random.randint(1500, 4000), # Price charged to patient
                        status="completed",
                    ...
                    db.add(lab_order)
                    db.commit()
                    print(f"   Created Lab Order for Patient ID: {patient.id} - Cost: {lab_order.cost}")
