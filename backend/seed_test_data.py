...
def seed_data():
    ...
    try:
        ...
        for dr in doctors_data:
            username = f"{dr['name'].replace(' ', '_').lower()}_{random.randint(100, 999)}"
            ...
            print(f"Created Doctor: {dr['name']} ({username}) - Commission: {dr['commission']}%")

        # 3. Create Patients & Treatments & Payments
        procedures_list = [
            ("Cleaning", 300), ("Filling", 500), ("Root Canal", 1500),
            ("Extraction", 400), ("Crown", 2000), ("Whitening", 2500),
            ("Implant", 8000), ("Braces", 12000)
        ]
        ...
        print(f"Login with any doctor username and password 'password123'")
