...
def reset_passwords():
    ...
    try:
        ...
        user = db.query(models.User).filter(models.User.username == username_email).first()
        if user:
        ...
        user_admin = db.query(models.User).filter(models.User.username == username_admin).first()
        if user_admin:
            new_hash_admin = auth.get_password_hash("1111")
            user_admin.hashed_password = new_hash_admin
            db.commit()
            print(f"Updated password for {username_admin} (Hash: {new_hash_admin[:10]}...)")
        else:
