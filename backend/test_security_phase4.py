...
def test_security_phase4():
    ...
    try:
        ...
        test_user = User(username="phase4_user", hashed_password="pw", is_2fa_enabled=False)
        db.add(test_user)
        db.commit()
        ...
        import uuid
        session = AuthService.create_session(db, test_user.id, str(uuid.uuid4()), "127.0.0.1", "TestAgent")
