...
class AnalyticsService:
    ...
    def get_doctor_ranking(self, period: str, metric: str) -> Dict[str, Any]:
        ...
        if metric == "revenue":
            ...
            query = self.db.query(
                ...
                func.sum(models.Treatment.cost).label("score")
    ...
    def get_dashboard_summary(self, period: str) -> Dict[str, Any]:
        ...
        # 1. Total Patients
        total_patients = self.db.query(models.Patient).filter(
            models.Patient.tenant_id == self.tenant_id
        ).count()
        
        # 2. New Patients
        new_patients = self.db.query(models.Patient).filter(
            models.Patient.tenant_id == self.tenant_id,
            date_filter_patient
        ).count()
        
        # 3. Appointments
        period_appointments = self.db.query(models.Appointment).join(models.Patient).filter(
            models.Patient.tenant_id == self.tenant_id,
            date_filter_app
        ).count()
        
        # 4. Revenue
        period_revenue = self.db.query(func.sum(models.Payment.amount)).join(models.Patient).filter(
            models.Patient.tenant_id == self.tenant_id,
            date_filter_pay
        ).scalar() or 0
    ...
    def get_clinic_summary(self) -> Dict[str, Any]:
        ...
        tenant = self.db.query(models.Tenant).filter(models.Tenant.id == self.tenant_id).first()
        if not tenant:
            return None
            
        users_count = self.db.query(models.User).filter(models.User.tenant_id == self.tenant_id).count()
        patients_count = self.db.query(models.Patient).filter(models.Patient.tenant_id == self.tenant_id).count()
    ...
    def get_ai_stats(self, period: str) -> Dict[str, Any]:
        ...
        query = self.db.query(models.AIUsageLog).filter(models.AIUsageLog.tenant_id == self.tenant_id)
        ...
        avg_latency = sum(log.response_time_ms for log in logs) / total_requests if total_requests > 0 else 0
        
        # Top tools
        tool_counts = {}
        for log in logs:
            if log.response_tool:
                tool_counts[log.response_tool] = tool_counts.get(log.response_tool, 0) + 1
    ...
    def compare_periods(self, period1: str, period2: str, metric: str) -> Dict[str, Any]:
        ...
        def get_range(p_name):
            t = date.today()
            if p_name == "this_week": return t - timedelta(days=7), t
            if p_name == "last_week": return t - timedelta(days=14), t - timedelta(days=7)
            if p_name == "this_month": return t - timedelta(days=30), t
            if p_name == "last_month": return t - timedelta(days=60), t - timedelta(days=30)
            # Default fallback
        ...
        def calculate_metric(s, e):
            if metric == "revenue":
                return self.db.query(func.sum(models.Payment.amount)).join(models.Patient).filter(
                    models.Patient.tenant_id == self.tenant_id,
                    models.Payment.date >= datetime.combine(s, datetime.min.time()),
                    models.Payment.date < datetime.combine(e, datetime.min.time())
                ).scalar() or 0
            elif metric == "patients":
                return self.db.query(models.Patient).filter(
                    models.Patient.tenant_id == self.tenant_id,
                    models.Patient.created_at >= datetime.combine(s, datetime.min.time()),
                    models.Patient.created_at < datetime.combine(e, datetime.min.time())
                ).count()
            else: # appointments
                return self.db.query(models.Appointment).join(models.Patient).filter(
                    models.Patient.tenant_id == self.tenant_id,
                    models.Appointment.date_time >= datetime.combine(s, datetime.min.time()),
                    models.Appointment.date_time < datetime.combine(e, datetime.min.time())
                ).count()
