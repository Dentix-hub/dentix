...
class FinancialVisibilityService:
    ...
    def get_visible_payments_query(self) -> Query:
        ...
        """
        base_query = self.db.query(Payment).join(Payment.patient).options(joinedload(Payment.patient)).filter(
    ...
    def get_visible_treatments_query(self) -> Query:
        ...
        Returns:
            SQLAlchemy Query filtered by user permissions
        """
        base_query = self.db.query(Treatment).join(Treatment.patient).filter(
