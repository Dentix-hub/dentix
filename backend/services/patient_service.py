...
class PatientService:
    ...
    def get_patient(self, db: Session = None, patient_id: int = None) -> Optional[models.Patient]:
        _db = db or self.db
        if not _db: raise ValueError("DB Session required")
        return _db.query(models.Patient).filter(models.Patient.id == patient_id).first()

    def get_patient_by_name(self, db: Session = None, tenant_id: int = None, name: str = None) -> Optional[models.Patient]:
        _db = db or self.db
        _tid = tenant_id or self.tenant_id
        if not _db: raise ValueError("DB Session required")
        
        return _db.query(models.Patient).filter(
            models.Patient.tenant_id == _tid,
            models.Patient.name == name
        ).first()

    def get_patient_file_details(self, name: str, db: Session = None, tenant_id: int = None) -> dict:
        """
        ...
        # Search patients
        patients = _db.query(models.Patient).filter(
            models.Patient.tenant_id == _tid,
        ...
        treatments = _db.query(models.Treatment).filter(
            models.Treatment.patient_id == patient.id
        ).order_by(desc(models.Treatment.date)).limit(5).all()
    ...
    def search_patients_by_name(self, query: str, db: Session = None, tenant_id: int = None) -> List[models.Patient]:
        """
    ...
    def get_patients_with_balance(self, db: Session = None, tenant_id: int = None) -> List[dict]:
        """
        ...
        for p in patients:
            ...
            if balance > 0:
                debtors.append({
                    "id": p.id,
                    "name": p.name,
                    "phone": p.phone.decrypt() if hasattr(p.phone, 'decrypt') else str(p.phone),
                    "balance": balance
                })
    ...
    def get_patient_summary_data(self, name: str, db: Session = None, tenant_id: int = None) -> dict:
        """
        ...
        payments = _db.query(models.Payment).filter(
            models.Payment.patient_id == patient.id
        ).all()
        
        total_cost = sum(t.cost for t in treatments) # Note: this only sums *recent* treatments from details? 
        # NO, details['treatments'] is limited to 5. We need FULL treatment history for balance.
        
        # Refetch full history for math
        all_treatments = _db.query(models.Treatment).filter(models.Treatment.patient_id == patient.id).all()
        ...
        summary_data = {
            "age": patient.age,
            "history": patient.medical_history.decrypt() if hasattr(patient.medical_history, 'decrypt') else str(patient.medical_history),
    ...
    def create_patient(
        ...
        _db = db or self.db
        _tid = tenant_id or self.tenant_id
        if not _db or not _tid: raise ValueError("DB and Tenant ID required")
        ...
        # 2. Duplicate Check
        existing = _db.query(models.Patient).filter(
            models.Patient.tenant_id == _tid,
        ...
        if existing:
            raise ValueError(f"Patient '{patient_data.name}' with phone '{patient_data.phone}' already exists.")
        ...
        new_patient = models.Patient(
            ...
            notes=f"{patient_data.notes or ''} [Gender: {patient_data.gender}]" if patient_data.gender else patient_data.notes
        )
    ...
    def update_patient(
        ...
        patient = _db.query(models.Patient).filter(
