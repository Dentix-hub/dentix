...
from backend.models.inventory import Warehouse, Material, Batch, StockItem, MaterialSession, StockMovement
...
class InventoryService:
    ...
    # --- WAREHOUSE ---
    def create_warehouse(self, data: schemas.WarehouseCreate, tenant_id: int, db: Session = None) -> Warehouse:
        db = self._get_db(db)
    ...
    def delete_warehouse(self, warehouse_id: int, tenant_id: int, db: Session = None):
        ...
        if has_stock:
            raise ValueError("Cannot delete warehouse with active stock. Please move or consume stock first.")
    ...
    # --- MATERIAL ---
    def create_material(self, data: schemas.MaterialCreate, tenant_id: int, db: Session = None) -> Material:
        db = self._get_db(db)
    ...
    def update_material(self, material_id: int, data: schemas.MaterialUpdate, tenant_id: int, db: Session = None) -> Material:
        db = self._get_db(db)
        mat = db.query(Material).filter(Material.id == material_id, Material.tenant_id == tenant_id).first()
        if not mat:
    ...
    # --- STOCK LOGIC ---
    def get_material_stock_summary(self, tenant_id: int, warehouse_id: Optional[int] = None, db: Session = None) -> List[schemas.MaterialStockSummary]:
        """
        ...
        for r in results:
            ...
            summary.append(schemas.MaterialStockSummary(
                material_id=r.id,
    ...
    def validate_stock(self,
                       ...
                       db: Session = None) -> Tuple[bool, float, str]:
        """
    ...
    # --- SESSION MANAGEMENT ---
    def get_active_session(self, stock_item_id: int, db: Session = None) -> Optional[MaterialSession]:
        db = self._get_db(db)
        return db.query(MaterialSession).filter(
            MaterialSession.stock_item_id == stock_item_id,
            MaterialSession.status == "ACTIVE"
        ).first()

    def open_session(self, stock_item_id: int, user_id: int, db: Session = None) -> MaterialSession:
        """
        ...
        if mat_type == "DIVISIBLE":
            ...
            if stock_item.quantity < ratio:
                raise ValueError(f"Insufficient stock to open session. Need {ratio} {mat.base_unit}, Have {stock_item.quantity}")
            ...
            if stock_item.quantity < 1:
                raise ValueError(f"Insufficient stock to open session. Need at least 1 unit, Have {stock_item.quantity}")
    ...
    def close_material_session_manually(self, session_id: int, user_id: int, db: Session = None):
        """
        ...
        total_usage = db.query(func.abs(func.sum(StockMovement.change_amount))).filter(
    ...
    def consume_stock(self,
                      ...
                      db: Session = None) -> List[StockMovement]:
        """
        ...
        if not warehouse_id:
            clinic_wh = db.query(Warehouse).filter(
                Warehouse.tenant_id == tenant_id, 
                Warehouse.type == "CLINIC"
            ).first()
            if clinic_wh:
        ...
        if mat.type == "NON_DIVISIBLE" and not float(quantity).is_integer() and not has_active_session_for_material:
             raise ValueError(f"Invalid Quantity: {mat.name} ({mat.type}) cannot be consumed in fractional amounts ({quantity}).")
        ...
        query = db.query(StockItem).join(Batch).filter(
            Batch.material_id == material_id,
            StockItem.tenant_id == tenant_id,
            ...
            if not session:
                ...
                if mat_type == 'DIVISIBLE':
                    if auto_open:
                        ...
                        # BLOCKING Trigger
                        raise ValueError(f"CONFIRM_OPEN_REQUIRED:{si.id}:{si.batch.material.name} - Batch {si.batch.batch_number}")
    ...
    def delete_material(self, material_id: int, tenant_id: int, db: Session = None):
        ...
        db.query(ProcedureMaterialWeight).filter(ProcedureMaterialWeight.material_id == material_id).delete()
        db.query(MaterialLearningLog).filter(MaterialLearningLog.material_id == material_id).delete()
        ...
        for b in batches:
            ...
            for si in stock_items:
                ...
                db.query(MaterialSession).filter(MaterialSession.stock_item_id == si.id).delete()
                # Then delete movements
                db.query(StockMovement).filter(StockMovement.stock_item_id == si.id).delete()
    ...
    def transfer_stock(self,
        ...
        if source_item.quantity < quantity:
            raise ValueError(f"Insufficient quantity. Available: {source_item.quantity}")

        # 2. Target Item (Find or Create)
        # Must match Batch & Material
        target_item = db.query(StockItem).filter(
        ...
        if mat.type == "NON_DIVISIBLE" and not float(quantity).is_integer():
             raise ValueError(f"Invalid Transfer Quantity: {mat.name} cannot be transferred in fractional amounts.")
