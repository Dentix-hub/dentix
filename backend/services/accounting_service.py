...
class AccountingService:
    ...
    def get_lab_costs_by_doctor(
        ...
        ).group_by(models.LabOrder.doctor_id).all()
    ...
    def calculate_doctor_dues(
        ...
        for doc_stat in doctors_analytics:
            ...
            doctor_dues.append({
                ...
                "collected": collected,
                "lab_cost": lab_cost,
                "net_revenue": doc_stat['net_revenue'], # Still useful to show potential
    ...
    def get_doctor_revenue_analytics(self, start: datetime, end: datetime) -> List[Dict[str, Any]]:
        """
        ...
        relevant_users = self.get_relevant_users(ROLES.DOCTOR_ROLES + ["admin", "super_admin"])
        ...
        .filter(
            ...
            models.Treatment.date >= start,
            models.Treatment.date <= end,
            models.Treatment.doctor_id.in_(relevant_user_ids) if relevant_user_ids else True
        ).group_by(models.Treatment.doctor_id, models.Treatment.patient_id).all()
        ...
        # 5. Payments
        payments_with_doctor = self.db.query(
        ...
        for doc_id, pat_id, amount in payments_with_doctor:
            if doc_id:
                doctor_payments_direct[doc_id] = doctor_payments_direct.get(doc_id, 0) + float(amount or 0)
        
        payments_all = self.db.query(
            models.Payment.patient_id,
            models.Payment.amount
        ).join(models.Patient, models.Payment.patient_id == models.Patient.id)\
        .filter(
        ...
        for user in relevant_users:
            ...
            stats = stats_map.get(doctor_id, {
                "treatment_count": 0, "gross_cost": 0.0, "total_discount": 0.0, "revenue": 0.0
            })
            ...
            for pat_id, amount in payments_all:
                ...
                if not has_doctor_payments:
                    payments_without_doctor[pat_id] = payments_without_doctor.get(pat_id, 0) + float(amount or 0)
    ...
    def get_doctor_details_data(self, doctor_id: int, start: datetime, end: datetime) -> Dict[str, Any]:
        """Get detailed breakdown for a specific doctor."""
        doctor = self.db.query(models.User).filter(models.User.id == doctor_id).first()
        if not doctor:
            return None

        treatments = self.db.query(models.Treatment, models.Patient.name)\
            .join(models.Patient, models.Treatment.patient_id == models.Patient.id)\
            .filter(
        ...
        lab_orders = self.db.query(models.LabOrder, models.Patient.name)\
    ...
    def update_staff_compensation_settings(
        ...
        commission: float, salary: float, fee: float
    ) -> bool:
        ...
        log_admin_action(
            self.db, current_user, "update", "compensation",
            target_user_id=user_id,
            old_value=old_values,
            new_value={"commission_percent": commission, "fixed_salary": salary, "per_appointment_fee": fee}
        )
        self.db.commit()
        return True
    ...
    def calculate_staff_dues(self, start: datetime, end: datetime, total_appointments: int) -> tuple[List[Dict[str, Any]], float]:
        """Calculate dues for staff based on fixed salary and appointment fees."""
        staff_members = self.db.query(models.User).filter(
            models.User.tenant_id == self.tenant_id,
            models.User.role.notin_(ROLES.DOCTOR_ROLES + ["super_admin", "admin"])
        ).all()
    ...
    def get_salary_status_for_month(self, month: str) -> Dict[str, Any]:
        ...
        employees = self.db.query(models.User).filter(
            models.User.tenant_id == self.tenant_id,
            models.User.role != "super_admin"
        ).all()

        payments = self.db.query(models.SalaryPayment).filter(
    ...
    def process_salary_payment(
        self, user_id: int, current_user: models.User, 
        month: str, amount: float, is_partial: bool, days_worked: int, notes: str
    ) -> Dict[str, Any]:
        """Record a salary payment."""
        existing = self.db.query(models.SalaryPayment).filter(
        ...
        log_admin_action(
            self.db, current_user, "create", "salary_payment",
            target_user_id=user_id,
            new_value={"month": month, "amount": amount, "is_partial": is_partial, "notes": notes}
        )
        self.db.commit()
    ...
    def remove_salary_payment(self, payment_id: int, current_user: models.User) -> bool:
        ...
        payment = self.db.query(models.SalaryPayment).filter(
        ...
        log_admin_action(
            self.db, current_user, "delete", "salary_payment",
    ...
    def update_employee_hire_date(self, user_id: int, hire_date_str: str, current_user: models.User) -> bool:
        ...
        try:
            ...
            log_admin_action(
                self.db, current_user, "update", "hire_date",
                target_user_id=user_id,
