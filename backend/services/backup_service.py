...
def update_backup_status(status: str, message: str, tenant_id: int = None):
    ...
    try:
        ...
        # 1. Update Status
        setting_status = db.query(models.SystemSetting).filter(models.SystemSetting.key == "backup_last_status").first()
        if not setting_status:
            setting_status = models.SystemSetting(key="backup_last_status", value=status)
            db.add(setting_status)
        else:
            setting_status.value = status

        # 2. Update Message
        setting_msg = db.query(models.SystemSetting).filter(models.SystemSetting.key == "backup_last_message").first()
        if not setting_msg:
        ...
        # 3. Update Time
        setting_time = db.query(models.SystemSetting).filter(models.SystemSetting.key == "backup_last_run").first()
...
def run_backup_task(refresh_token: str, db_url: str, tenant_id: int = None, tenant_name: str = None):
    """
    ...
    try:
        ...
        update_backup_status("processing", f"Export success ({size_kb:.2f} KB). Uploading...", tenant_id)
        ...
        error_str = str(e)
        if "HttpError 401" in error_str or "HttpError 403" in error_str or "invalid_grant" in error_str.lower():
            logger.warning("Authentication failed. Disconnecting Google Drive.")
            try:
                db = SessionLocal()
                # Auto-disconnect
                db.query(models.SystemSetting).filter(models.SystemSetting.key == "google_refresh_token_super_admin").delete()
                # Reset status
                db.query(models.SystemSetting).filter(models.SystemSetting.key.in_([
                    "backup_last_status", "backup_last_message", "backup_last_run"
                ])).delete(synchronize_session=False)
                db.commit()
                db.close()
                update_backup_status("failed", "تم فك الربط تلقائياً لانتهاء الصلاحية. يرجى إعادة الربط.", tenant_id)
                return
