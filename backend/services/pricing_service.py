...
from backend.models import (
    ...
    InsuranceProvider, User, Patient
)
...
class PricingService:
    ...
    def get_default_price_list(self) -> Optional[PriceList]:
        ...
        return self.db.query(PriceList).filter(
    ...
    def get_all_prices_for_procedure(
        self,
        procedure_id: int
    ) -> List[Dict[str, Any]]:
        """Get price from all price lists for a procedure."""
        items = self.db.query(PriceListItem).join(PriceList).filter(
            PriceListItem.procedure_id == procedure_id,
            PriceList.tenant_id == self.tenant_id,
            PriceList.is_active == True
        ).all()
