...
class CostEngine:
    ...
    def get_material_average_cost(self, material_id: int) -> float:
        ...
        # 1. Active Stock
        stock_query = (
            self.db.query(StockItem.quantity, Batch.cost_per_unit, Material.packaging_ratio)
        ...
        if last_batch:
            ...
            effective_ratio = mat.packaging_ratio if mat.packaging_ratio and mat.packaging_ratio > 0 else 1.0
    ...
    def calculate_procedure_cost(self, procedure_id: int) -> Dict[str, Any]:
        ...
        actual_margin_percent = (actual_margin / current_price * 100) if current_price > 0 else 0.0
    ...
    def calculate_all_procedures_costs(self) -> List[Dict[str, Any]]:
        ...
        """
        procedures = self.db.query(Procedure).filter(Procedure.tenant_id == self.tenant_id).all()
        results = []
            ...
            try:
                ...
                if "error" not in analysis:
                    results.append({
                        ...
                        "cost": analysis["total_actual_cost"], # Using AI/Actual cost as the source of truth
