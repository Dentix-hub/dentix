...
class InventoryLearningService:
    ...
    def close_session(self, session_id: int, total_consumed: float, user_id: int):
        ...
        try:
            session = self.db.query(inv_models.MaterialSession).options(
                joinedload(inv_models.MaterialSession.stock_item).joinedload(inv_models.StockItem.batch)
            ).filter(inv_models.MaterialSession.id == session_id).first()
            ...
            if not session.stock_item.batch:
                 ...
                 raise ValueError("CRITICAL: StockItem has no linked Batch (Data Corruption)")
        ...
        if session.doctor_id:
            query = query.filter(clinical_models.Treatment.doctor_id == session.doctor_id)
        ...
        if not treatments:
            ...
            self._log_learning(session, total_consumed, {"error": "No treatments found in window"})
        ...
        procs = self.db.query(clinical_models.Procedure).filter(
        ...
        # Get Weights
        weights = self.db.query(inv_models.ProcedureMaterialWeight).filter(
        ...
        for t in treatments:
            if not t.procedure: continue
            
            pid = proc_map.get(t.procedure)
            if not pid: continue # Procedure not configured in DB?
            
            w_obj = weight_map.get(pid)
            weight_val = w_obj.weight if w_obj else 1.0 # Default weight 1.0 if not defined
        ...
        if total_weight_score == 0:
             self._log_learning(session, total_consumed, {"error": "Total weight score is zero"})
             self.db.commit()
             return
    ...
    def get_suggested_materials(self, procedure_id: int, doctor_id: int = None, patient_age: int = None) -> List[Dict]:
        """
        ...
        """
        # 1. Get Base Weights
        weights = self.db.query(inv_models.ProcedureMaterialWeight).options(
            joinedload(inv_models.ProcedureMaterialWeight.material)
        ).filter(
