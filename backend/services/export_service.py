...
def export_tenant_data(db: Session, tenant_id: int) -> Dict:
    ...
    export_data["data"]["users"] = [
        model_to_dict(u, exclude_fields=["active_session_id"])
        for u in users
    ]
    ...
    # 5. ToothStatus
    tooth_statuses = db.query(models.ToothStatus).join(
        models.Patient
    ).filter(
        models.Patient.tenant_id == tenant_id
    ).all()
    ...
    # 7. Attachments
    attachments = db.query(models.Attachment).join(
        models.Patient
    ).filter(
        models.Patient.tenant_id == tenant_id
    ).all()
    ...
    export_data["data"]["salary_payments"] = [model_to_dict(sp) for sp in salary_payments]
    
    # 11. Laboratories
    laboratories = db.query(models.Laboratory).filter(
    ...
    lab_payments = db.query(LabPayment).filter(
    ...
    # 15. SavedMedications
    saved_medications = db.query(models.SavedMedication).filter(
        models.SavedMedication.tenant_id == tenant_id
    ).all()
    export_data["data"]["saved_medications"] = [model_to_dict(sm) for sm in saved_medications]
    
    # 16. InsuranceProviders
    from ..models.price_list import InsuranceProvider, PriceList, PriceListItem
    insurance_providers = db.query(InsuranceProvider).filter(
        InsuranceProvider.tenant_id == tenant_id
    ).all()
    export_data["data"]["insurance_providers"] = [model_to_dict(ip) for ip in insurance_providers]
    ...
    if price_list_ids:
        ...
        export_data["data"]["price_list_items"] = [model_to_dict(pli) for pli in price_list_items]
    else:
    ...
    if stock_item_ids:
        material_sessions = db.query(models.MaterialSession).filter(
            models.MaterialSession.stock_item_id.in_(stock_item_ids)
        ).all()
        export_data["data"]["material_sessions"] = [model_to_dict(ms) for ms in material_sessions]
    else:
    ...
    if stock_item_ids:
        ...
        export_data["data"]["stock_movements"] = [model_to_dict(sm) for sm in stock_movements]
    else:
        export_data["data"]["stock_movements"] = []
    
    # 25. ProcedureMaterialWeights
    procedure_material_weights = db.query(models.ProcedureMaterialWeight).filter(
        models.ProcedureMaterialWeight.tenant_id == tenant_id
    ).all()
    export_data["data"]["procedure_material_weights"] = [model_to_dict(pmw) for pmw in procedure_material_weights]
