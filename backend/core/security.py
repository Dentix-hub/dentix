...
class EncryptionManager:
        ...
        if self._key:
            if not self.validate_key(self._key):
                raise InvalidKeyError(
                    "Invalid encryption key format. Must be 32-byte base64 encoded."
                )
            self._fernet = Fernet(
    ...
    def decrypt(
        self, encrypted_data: str, allow_plaintext_fallback: bool = True
    ) -> str:
        """
    ...
    def rotate_key(self, new_key: str, data_list: list[str]) -> list[str]:
        ...
        try:
            ...
            for encrypted_data in data_list:
                ...
                new_encrypted = new_fernet.encrypt(plaintext.encode("utf-8")).decode(
                    "utf-8"
                )
