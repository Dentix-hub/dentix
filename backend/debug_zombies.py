...
def check_zombie_tenants():
    ...
    try:
        ...
        print(f"{'ID':<5} | {'Name':<20} | {'Active?':<8} | {'Deleted?':<8} | {'Plan':<10}")
        print("-" * 60)
        
        tenants = db.query(models.Tenant).all()
        for t in tenants:
            is_deleted = getattr(t, 'is_deleted', 'N/A')
            print(f"{t.id:<5} | {t.name[:20]:<20} | {str(t.is_active):<8} | {str(is_deleted):<8} | {t.plan}")

        print("\n\n--- USER LOGIN CAPABILITY REPORT ---")
        print(f"{'User':<15} | {'Role':<10} | {'Tenant':<15} | {'Can Login?':<10} | {'Reason'}")
        print("-" * 80)
        ...
        for u in users:
            ...
            print(f"{u.username:<15} | {u.role:<10} | {str(u.tenant.name if u.tenant else 'None'):<15} | {can_login:<10} | {reason}")
