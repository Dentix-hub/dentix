...
from backend.models import User, Tenant, SystemSetting, AuditLog
...
def test_enterprise_features():
    ...
    try:
        ...
        if not maintenance:
            ...
            print(f" - PASS: Maintenance mode setting exists (Value: {maintenance.value})")
        ...
        try:
            db.execute(text("SELECT is_deleted, deleted_at, is_active FROM users LIMIT 1"))
        ...
        for log in logs:
            print(f"   * [{log.action}] {log.entity_type} (Admin: {log.performed_by_username})")
