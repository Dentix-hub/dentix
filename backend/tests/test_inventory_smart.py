...
def test_db():
    ...
    # 2. Create Materials
    mat1 = models.Material(name="Composite A1", base_unit="capsule", type="NON_DIVISIBLE", tenant_id=tenant.id)
    mat2 = models.Material(name="Bonding Agent", base_unit="ml", type="DIVISIBLE", tenant_id=tenant.id)
...
def test_smart_suggestions(test_db):
    ...
    response = client.get(f"/api/v1/inventory/smart/suggestions/{proc.id}", headers=headers)
...
def test_check_availability(test_db):
    ...
    res = client.post("/api/v1/inventory/smart/check-availability", json=payload, headers=headers)
    ...
    res = client.post("/api/v1/inventory/smart/check-availability", json=payload_high, headers=headers)
    data = res.json()["data"]
    ...
    res = client.post("/api/v1/inventory/smart/check-availability", json=payload_none, headers=headers)
    data = res.json()["data"]
