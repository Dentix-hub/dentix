...
from backend.models import (
    PriceList, PriceListItem, Procedure, Treatment, 
    User, PriceListType
)
...
class TestPricingService:
    ...
    def test_fallback_to_default_list(self):
        ...
        mock_db.query.return_value.filter.return_value.first.side_effect = [None, MagicMock(final_price=500.0)]
...
class TestAIPriceContext:
    ...
    def test_doctor_restricted_access(self):
        ...
        with patch('backend.services.pricing_service.PricingService.get_available_price_lists', return_value=[pl1, pl2]):
            context = AIPriceContext.from_user(mock_db, user, 1)
    ...
    def test_system_prompt_injection(self):
        ...
        context = AIPriceContext(
            user_id=1, user_role="doctor", tenant_id=1,
            allowed_price_list_ids={1}
        )
