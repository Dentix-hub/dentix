...
def test_token_validity(super_admin_headers):
    ...
    try:
        ...
        assert payload["sub"] == "test_super_admin"
...
def test_get_tenants_pagination(client, db, super_admin_headers):
    ...
    response = client.get("/admin/tenants?skip=0&limit=10", headers=super_admin_headers)
    assert response.status_code == 200, f"Status {response.status_code}. Detail: {response.text}"
    data = response.json()
    assert len(data) == 10
    
    # 3. Request Page 2 (Limit 10)
    response = client.get("/admin/tenants?skip=10&limit=10", headers=super_admin_headers)
    assert response.status_code == 200, f"Status {response.status_code}. Detail: {response.text}"
    data = response.json()
    assert len(data) >= 5


def test_update_tenant_plan_upgrade(client, db, super_admin_headers):
    ...
    response = client.put(f"/admin/tenants/{tenant.id}", json=payload, headers=super_admin_headers)
...
def test_soft_delete_tenant(client, db, super_admin_headers):
    ...
    response = client.delete(f"/admin/tenants/{tenant.id}", headers=super_admin_headers)
    assert response.status_code == 200, f"Status {response.status_code}. Detail: {response.text}"
    
    # 3. Verify Soft Delete (Not Hard Delete)
    db.expire_all()  # invalidate cache
    updated_tenant = db.query(Tenant).filter(Tenant.id == tenant.id).first()
    updated_user = db.query(User).filter(User.id == admin_user.id).first()
