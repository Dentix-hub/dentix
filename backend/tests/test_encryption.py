...
def test_encryption_verification():
    ...
    try:
        ...
        result = db.execute(text("SELECT phone FROM patients WHERE id = :id"), {"id": patient_id}).fetchone()
        raw_phone = result[0]
        print(f"Raw SQL Phone: {raw_phone}")
        
        assert raw_phone != test_phone
        assert "gAAAA" in raw_phone or len(raw_phone) > 20 # Basic Fernet signature check
