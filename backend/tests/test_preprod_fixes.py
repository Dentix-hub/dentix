...
def test_soft_delete_patient(db):
    tenant_id = 999999
    
    # 1. Create Patient
    p_data = schemas.PatientCreate(name="Soft Delete Test", age=30, phone="ENCRYPTED")
    patient = crud.create_patient(db, p_data, tenant_id)
    assert patient.id is not None
    assert patient.is_deleted == False
    
    # 2. Soft Delete
    crud.delete_patient(db, patient.id, tenant_id)
    
    # 3. Verify
    # Should not find it via get_patient
    deleted = crud.get_patient(db, patient.id, tenant_id)
    assert deleted is None
    
    # But should still exist in DB (Direct Check)
    raw_patient = db.query(models.Patient).filter(models.Patient.id == patient.id).first()
    assert raw_patient.is_deleted == True
    assert raw_patient.deleted_at is not None

def test_soft_delete_appointment(db):
    tenant_id = 999999
    ...
    appt = crud.create_appointment(db, a_data)
    
    # 2. Soft Delete
    crud.delete_appointment(db, appt.id, tenant_id)
    
    # 3. Verify
    # Should be filtered out from list
    appts = crud.get_appointments(db, tenant_id)
    ids = [a.id for a in appts]
    assert appt.id not in ids
    
    # Direct DB Check
    raw_appt = db.query(models.Appointment).filter(models.Appointment.id == appt.id).first()
...
def test_double_booking_prevention(db):
    ...
    crud.create_appointment(db, a1_data)
    ...
    with pytest.raises(ValueError) as excinfo:
        crud.create_appointment(db, a2_data)
