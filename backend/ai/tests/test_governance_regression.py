...
from backend.core.permissions import Role, Permission, has_permission, ROLE_PERMISSIONS
from backend.ai.policy.execution_policy import policy_engine, PolicyRule
...
class TestGovernanceRegression(unittest.IsolatedAsyncioTestCase):
    ...
    def test_service_layer_rbac_enforcement(self):
        ...
        try:
            service.create_patient(
                PatientCreate(name="Test", phone="123", age=30), 
                creator_role="doctor"
            )
    ...
    async def test_kill_switch_enforcement(self):
        ...
        with patch("backend.core.config.is_ai_disabled", return_value=False):
            ...
            exec._handlers["patient"].search_patients = AsyncMock(return_value={"success": True})
    ...
    async def test_audit_log_persistence(self):
        ...
        service.agent.process = AsyncMock(return_value={
            "tool": "response", 
            "message": "Hi", 
            "success": True
        })
