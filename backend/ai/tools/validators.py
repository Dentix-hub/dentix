...
def validate_tool_access(tool: Any, user_role: str):
    ...
    if user_role not in tool.allowed_roles:
        raise ValidationError(f"Permission invalid for tool '{tool.name}'. Required: {tool.allowed_roles}")
    ...
    if "amount" in params or "cost" in params:
        ...
        for field in ["amount", "cost", "paid_amount"]:
            if field in params and params[field]:
                ...
                if not num_match:
                    raise ValidationError(f"Invalid numeric value for field '{field}': {params[field]}")
