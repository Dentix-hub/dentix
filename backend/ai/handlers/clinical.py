...
class ClinicalHandler(BaseHandler):
    ...
    async def get_lab_orders(self, params: Dict) -> Dict:
        ...
        return {
            "message": f"طلبات المعمل: {len(orders)} طلب",
            "orders": orders
        }
    ...
    async def record_medical_note(self, params: Dict) -> Dict:
        ...
        patient = await self._find_single_patient(patient_name) if patient_name else None
    ...
    async def update_tooth_status(self, params: Dict) -> Dict:
        ...
        if isinstance(patient, dict): return patient # Error/List
        if not patient: return {"error": "not_found", "message": "المريض غير موجود"}
    ...
    async def add_treatment_voice(self, params: Dict) -> Dict:
        ...
        patient = await self._find_single_patient(patient_name)
        if isinstance(patient, dict): return patient
        
        if not patient:
            # Check if it was a price check
            if cost <= 0:
                # Redirect logic (simplified)
                return {"message": f"سعر {procedure}: (تحتاج لتوجيه لسجل المالية)", "action": "redirect_finance"}
        ...
        return {
            ...
            "files_updated": ["treatments", "payments"] if paid_amount else ["treatments"]
        }
    ...
    async def parse_medical_dictation(self, params: Dict) -> Dict:
        ...
        if not text: return {"error": "missing_text", "message": "النص فارغ"}
        ...
        try:
            ...
            if params.get("patient_name"):
                ...
                if hasattr(p, "id"): patient_info = {"id": p.id, "name": p.name}
    ...
    async def _find_single_patient(self, name: str):
        ...
        if len(patients) == 1: return patients[0]
