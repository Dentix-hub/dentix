...
class AdminHandler(BaseHandler):
    ...
    async def get_doctor_ranking(self, params: Dict) -> Dict:
        ...
        # Format message
        if rankings:
            metric_label = {"revenue": "إيرادات", "patients": "مرضى"}.get(metric, metric)
    ...
    async def compare_periods(self, params: Dict) -> Dict:
        ...
        metric_label = {"revenue": "الإيرادات", "patients": "المرضى الجدد", "appointments": "المواعيد"}.get(metric, metric)
        unit = " جنيه" if metric == "revenue" else ""
    ...
    async def get_ai_stats(self, params: Dict) -> Dict:
        ...
        tool_text = "\n".join([f"• {t['name']}: {t['count']} مرة" for t in data['top_tools']])
