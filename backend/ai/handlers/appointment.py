...
class AppointmentHandler(BaseHandler):
    ...
    async def get_appointments(self, params: Dict) -> Dict:
        ...
        # Get appointments
        appointments = self.db.query(models.Appointment).join(
            ...
            models.Appointment.date_time < datetime.combine(target_date + timedelta(days=1), datetime.min.time())
        ).all()
        
        if not appointments:
            return {
                "message": f"لا توجد مواعيد في {target_date}",
                "appointments": []
            }
    ...
    async def find_available_slots(self, params: Dict) -> Dict:
        ...
        existing = self.db.query(models.Appointment).join(
            models.Patient
        ).filter(
            models.Patient.tenant_id == self.tenant_id,
            models.Appointment.date_time >= datetime.combine(target_date, datetime.min.time()),
    ...
    async def smart_book_appointment(self, params: Dict) -> Dict:
        ...
        if time_str:
            try:
                if ":" in str(time_str):
                    ...
                    hour = int(time_str)
                    minute = 0
                preferred_time = datetime.combine(target_date, datetime.min.time().replace(hour=hour, minute=minute))
        ...
        # Check for conflicts
        if preferred_time:
            conflict = self.db.query(models.Appointment).join(
                models.Patient
            ).filter(
                models.Patient.tenant_id == self.tenant_id,
            ...
            if conflict:
                # Find alternatives
                available = await self.find_available_slots({"date": date_str, "period": "any"})
    ...
    async def create_appointment(self, params: Dict) -> Dict:
        ...
        elif "بعد" in date_str and ("يوم" in date_str or "أيام" in date_str or "ايام" in date_str):
        ...
        try:
            ...
            target_datetime = datetime.combine(target_date, datetime.min.time().replace(hour=10, minute=0))
