...
class AIAgent:
    ...
    async def process(
        ...
        try:
            ...
            if detected and detected.skip_llm:
                logger.info(f"Bypassing LLM - handling detected intent: {detected.intent}")
        ...
        if tenant_id:
            try:
                results = knowledge_store.search(safe_input, tenant_id=tenant_id, n_results=2)
                if results:
        ...
        system_prompt = build_system_prompt(last_entity, rag_context, scribe_mode=scribe_mode)
        ...
        try:
            ...
            if "parameters" in result and "patient_name" in result["parameters"]:
                ...
                if p_name:
                    state_manager.update_session(tenant_id, user_id, {
    ...
    def _enforce_scribe_mode(self, result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        SCRIBE_ALLOWED_TOOLS = [
            ...
            "get_procedure_price", "search_patients"
        ]
        
        tool_name = result.get("tool")
        
        if tool_name not in SCRIBE_ALLOWED_TOOLS:
            logger.warning(f"Scribe Mode: Blocked tool '{tool_name}' - returning note recording suggestion")
    ...
    def _handle_detected_intent(self, detected, original_input: str) -> Dict[str, Any]:
        ...
        return {
            "tool": "response",
            "message": "من فضلك وضّح طلبك."
        }

    def build_system_prompt(self, last_entity: Optional[str] = None, rag_context: Optional[str] = None) -> str:
