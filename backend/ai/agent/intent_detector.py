...
class IntentDetector:
    ...
    def detect(self, user_input: str, context: Optional[Dict] = None) -> Optional[DetectedIntent]:
        """
    ...
    def _extract_patient_name(self, text: str) -> Optional[str]:
        ...
        for kw in self.PATIENT_REGISTRATION_KEYWORDS:
            if kw in text:
                ...
                name_words = [w for w in words if not re.match(r"^\d+$", w) and w not in ["عمره", "سنه", "ورقمه", "رقمه"]]
                if name_words:
