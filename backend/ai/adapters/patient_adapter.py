...
from backend.ai.response_models import PatientFileResponse, PatientListResponse, BaseToolResponse
...
class PatientToolAdapter:
    ...
    async def summarize_patient(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not result["found"]:
             raise AINotFoundError(result.get("message", "المريض غير موجود"), code="patient_not_found")
        ...
        if result.get("multiple"):
             return {
                 ...
                 "patients": [ {"id": p.id, "name": p.name} for p in result.get("patients", []) ]
             }
    ...
    async def create_patient(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AIValidationError(f"Invalid patient data: {str(e)}", code="validation_error")
    ...
    async def get_patients_with_balance(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        detail_msg = ""
        if patients:
            detail_msg = "\n" + "\n".join([f"• {p['name']}: {p['balance']:,.0f} جنيه" for p in patients[:15]])
