...
def fix_null_values():
    ...
    try:
        ...
        db.execute(text(f"UPDATE patients SET is_deleted = {bool_false} WHERE is_deleted IS NULL"))
        
        print("🔄 Updating NULL is_deleted in 'appointments' to False...")
        db.execute(text(f"UPDATE appointments SET is_deleted = {bool_false} WHERE is_deleted IS NULL"))
