...
def test_calculate_bom_cost():
    ...
    bom = [
        ProcedureMaterialWeight(material_id=1, weight=2.0),
        ProcedureMaterialWeight(material_id=2, weight=0.5)
    ]
    ...
    # Execution
    # Mocking the internal method to avoid complex DB queries
    original_get_avg = engine.get_material_average_cost
    
    def mock_get_avg(material_id):
        if material_id == 1: return 10.0 # Resin
        if material_id == 2: return 50.0 # Bond
        return 0.0
        
    engine.get_material_average_cost = mock_get_avg

    # Expected: (2 * 10) + (0.5 * 50) = 20 + 25 = 45.0
